...
import { getInsights } from "@/logic/decisions";
...
export default function DecisionsPage() {
  const insights = useLiveQuery(async () => {
    await Promise.all([db.sales.count(), db.products.count(), db.movements.count()]);
    return getInsights();
  }, []);
  ...
  const groups = ORDER
    .map((level) => ({ level, items: insights.filter((i) => i.level === level) }))
  ...
  return (
    <div>
      ...
      {insights.length === 0 ? (
        <EmptyState icon="✅" title="Nada exige sua atenção agora"
          detail="Conforme você registrar vendas, entradas e produções, as recomendações aparecem aqui sozinhas." />
      ) : (
      ...
      <Card className="mt-6 bg-ink-50">
        <p className="text-xs leading-relaxed text-ink-600">
          <strong>Como as recomendações são calculadas:</strong> comparamos o estoque com o mínimo
          cadastrado e com a média de vendas e consumo dos últimos 30 dias; o preço de compra dos
          insumos dos últimos 30 dias com os 90 anteriores; as vendas de cada produto nos últimos
          30 dias com os 30 anteriores; e o rendimento de cada produção com a média das últimas.
          Ajuste os parâmetros em{" "}
          <Link to="/configuracoes" className="font-semibold text-leaf-700 underline">Configurações</Link>.
        </p>
      </Card>
    </div>
  );
